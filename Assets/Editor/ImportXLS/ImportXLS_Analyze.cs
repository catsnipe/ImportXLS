...
public partial class ImportXLS : EditorWindow
{
    ...
    static bool analyzeEnumsAndConsts(SheetEntity report)
    {
        ...
        foreach (var pair in posList)
        {
            ...
            if (pair.Key.Contains(TRIGGER_CLASS) == true)
            {
                continue;
            }
            ...
            for (int r = top.R+1; r < grid.GetUpperBound(0)+1; r++)
            {
                ...
                {
                    // 同じメンバ名
                    log_error(MSG_SAMEMEMBER, sheet.SheetName, GetXLS_RC(r, c), name);
                    return false;
                }
   ...
   static void analyzeClassComments(SheetEntity report)
    {
        ...
        if (posList.ContainsKey(TRIGGER_CLASS) == false)
        {
            return;
        }

        PosIndex id = posList[TRIGGER_CLASS];
        int c = id.C+1;

        for (int r = id.R; r < grid.GetUpperBound(0)+1; r++)
        {
            string name = grid[r, c];
            if (string.IsNullOrEmpty(name) == true)
            {
    ...
    static bool analyzeClasses(SheetEntity report)
    {
        ...
        for (c = id.C; c < grid.GetUpperBound(1)+1; c++)
        {
            ...
            for (r = id.R-1; r > -1; r--)
            {
                ...
                if (CheckSignComment(classname) == true)
                {
                    // comment
                }
                else
                if (classname.IndexOf(SIGN_COMMA) >= 0)
                {
                    // other class (sub class の宣言の必要なし)
                }
                else
                {
            ...
            {
                // 最終行で型が取れない
                log_error(MSG_TYPE_NOTFOUND, sheet.SheetName, GetXLS_RC(id.R, c), "");
                return false;
            }
...
//typestr = typestr.Insert(0, report.TableName);
            ...
            else
            {
                ...
                {
                    // 配列ではないのに同じメンバ名
                    log_error(MSG_SAMEMEMBER, sheet.SheetName, GetXLS_RC(id.R, c), name);
                    return false;
                }
                else
                {
